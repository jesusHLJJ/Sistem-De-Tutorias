    ...
    <div class="w-11/12 max-w-7xl mx-auto py-8 flex flex-col items-center flex-grow">
        ...
        <div class="bg-[#044C26]/90 w-full rounded-xl shadow-2xl p-6 md:p-8 backdrop-blur-sm">
            ...
            <div class="flex flex-wrap justify-between items-center mb-4 sm:mb-6 gap-3">
                <h3 class="text-white font-montserrat font-bold text-lg sm:text-xl md:text-2xl">Asignación de Materias</h3>
                
                <a href="{{ route('admin.materias.index') }}" class="bg-[#A3E635] hover:bg-[#84cc16] text-[#044C26] font-bold py-2 px-3 sm:px-4 md:px-6 rounded-lg shadow-lg transform hover:-translate-y-1 transition duration-200 flex items-center gap-2 border-none text-xs sm:text-sm md:text-base no-underline">
                    <i class="fa-solid fa-list-check text-base sm:text-lg md:text-xl"></i>
                    <span class="hidden sm:inline">Administrar Materias</span><span class="sm:hidden">Administrar</span>
                </a>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-[#0A8644] text-white font-montserrat uppercase text-xs sm:text-sm">
                        <tr>
                            ...
                            <th class="py-3 px-3 sm:py-4 sm:px-6">Clave Grupo</th>
                            <th class="py-3 px-3 sm:py-4 sm:px-6">Carrera</th>
                            <th class="py-3 px-3 sm:py-4 sm:px-6 text-center"># Materias</th>
                        ...
                        @foreach ($grupos as $grupo)
                            <tr class="bg-white/10 hover:bg-white/20 border-b border-white/10 transition-colors materia-row"
                                ...
                                <td class="py-2 px-3 sm:py-3 sm:px-6 text-center">
                                    <span class="badge {{ $grupo->materias_count > 0 ? 'bg-green-500' : 'bg-red-500' }} text-white py-1 px-2 rounded-full text-xs">
                                ...
                                <td class="py-2 px-3 sm:py-3 sm:px-6 text-center">
                                    <div class="flex item-center justify-center gap-1 sm:gap-2">
                                        <button class="bg-yellow-500 hover:bg-yellow-600 text-white w-8 h-8 sm:w-9 sm:h-9 rounded-lg flex items-center justify-center transition shadow-md no-underline text-xs sm:text-sm btn-edit"
                                           data-id="{{ $grupo->id_grupo }}"
                                           data-clave="{{ $grupo->clave_grupo }}" 
                                           data-carrera-id="{{ $grupo->id_carrera }}"
                                           data-materias="{{ json_encode($grupo->materias->pluck('id_materia')) }}"
                                           title="Editar Materias">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </button>

                                        <button class="bg-red-500 hover:bg-red-600 text-white w-8 h-8 sm:w-9 sm:h-9 rounded-lg flex items-center justify-center transition shadow-md no-underline text-xs sm:text-sm btn-delete"
                                           data-id="{{ $grupo->id_grupo }}"
                                           data-clave="{{ $grupo->clave_grupo }}"
                                           data-count="{{ $grupo->materias_count }}"
                                           title="Desasignar Todas">
