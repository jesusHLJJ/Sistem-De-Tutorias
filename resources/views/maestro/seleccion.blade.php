...
<head>
    ...
    <title>Opciones del Alumno - {{ $alumno->nombre }}</title>
...
<body class="font-montserrat bg-cover bg-center bg-fixed min-h-screen md:h-[125vh] md:overflow-hidden flex flex-col bg-[url('{{ asset('multimedia/fondo.jpg') }}')] md:[zoom:80%]">
    ...
    <div class="bg-tec-green shadow-[0_12px_14px_rgba(0,0,0,0.25)] h-10 md:h-12 shrink-0 flex items-center justify-center relative z-20">
        <h2 class="text-white font-bold text-base md:text-2xl tracking-wide">MAESTRO</h2>
    </div>
    ...
    <div class="flex flex-1 overflow-hidden relative">
        ...
        <div id="content-area" class="flex-1 p-4 md:p-10 flex justify-center items-start overflow-y-auto md:ml-64">
            <div class="bg-tec-green-dark/90 backdrop-blur-md rounded-xl md:rounded-2xl p-6 md:p-10 w-full max-w-4xl shadow-[0_8px_32px_rgba(0,0,0,0.3)] min-h-[500px] flex flex-col">
                
                <div class="text-center mb-8 border-b-2 border-white/30 pb-4">
                    <h2 class="text-white text-xl md:text-3xl font-bold">
                        OPCIONES DEL ALUMNO
                    </h2>
                    <p class="text-white/70 text-lg mt-2">
                        Estudiante: <span class="font-bold text-white">{{ $alumno->nombre }} {{ $alumno->ap_paterno }}</span>
                    </p>
                </div>

                <div class="flex-1 flex flex-col justify-center items-center text-center space-y-8">
                    <div class="bg-white/10 p-6 rounded-full inline-block mb-4">
                        <i class="fa-solid fa-user-graduate text-6xl text-white/80"></i>
                    </div>
                    
                    <div class="max-w-2xl space-y-6 text-white/90 text-lg">
                        <p>
                            Seleccione una opción del menú lateral izquierdo para gestionar la información del alumno.
                        </p>
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-left mt-8">
                            <div class="bg-white/5 p-4 rounded-lg border border-white/10">
                                <h3 class="font-bold text-hover-pink mb-2"><i class="fa-solid fa-id-card mr-2"></i>Ficha de Identificación</h3>
                                <p class="text-sm text-white/70">Consulte los datos personales, antecedentes académicos y socioeconómicos.</p>
                            </div>
                            
                            <div class="bg-white/5 p-4 rounded-lg border border-white/10">
                                <h3 class="font-bold text-hover-pink mb-2"><i class="fa-solid fa-clipboard-list mr-2"></i>Habilidades de Estudio</h3>
                                <p class="text-sm text-white/70">Revise los resultados de las encuestas de organización y motivación.</p>
                            </div>
                            
                            <div class="bg-white/5 p-4 rounded-lg border border-white/10">
                                <h3 class="font-bold text-hover-pink mb-2"><i class="fa-solid fa-comments mr-2"></i>Solicitudes de Asesoría</h3>
                                <p class="text-sm text-white/70">Acceda al historial de solicitudes y detalles de asesorías.</p>
                            </div>
                            
                            <div class="bg-white/5 p-4 rounded-lg border border-white/10">
                                <h3 class="font-bold text-hover-pink mb-2"><i class="fa-solid fa-share-nodes mr-2"></i>Canalización</h3>
                                <p class="text-sm text-white/70">Gestione las derivaciones a otras áreas de apoyo institucional.</p>
                            </div>
                        </div>
                    </div>
                </div>
